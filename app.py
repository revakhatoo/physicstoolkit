...
if tool == "Reading based":
    ...
    with input_col:
        ...
        # --- NEW: Least Count Input for Type B Uncertainty ---
        least_count = col_lc.number_input("Least Count (Instrument Error)", value=0.10, min_value=0.0, format="%.4f")
        unit_input = col_unit.text_input("Unit (optional)", "")
        ...
        if not clean_df.empty and len(clean_df) > 1:
            ...
            # --- NEW: Combined Error Propagation ---
            total_abs_error = np.sqrt(std_error**2 + least_count**2)
            rel_error = total_abs_error / abs(mean_val) if mean_val != 0 else 0
            pct_error = rel_error * 100
            
            st.success(f"**Mean ($\\bar{{x}}$):** {mean_val:.4g}")
            st.info(f"**Std Deviation ($\\sigma$):** {std_dev:.4g}")
            st.info(f"**Standard Error (Type A):** {std_error:.4g}")
        else:
            st.warning("Please enter at least two data points.")
            mean_val, std_dev, std_error, total_abs_error, rel_error, pct_error = 0, 0, 0, 0, 0, 0
    ...
    with display_col:
        if not clean_df.empty and len(clean_df) > 1:
            ...
            # --- NEW: Advanced Propagation Math Outputs ---
            st.markdown("### Total Uncertainty Propagation")
            
            metrics_col1, metrics_col2 = st.columns(2)
            with metrics_col1:
                st.markdown(f"**Absolute Uncertainty ($\\Delta x$):** {total_abs_error:.4g}")
                st.markdown(f"**Relative Error:** {rel_error:.4g}")
                st.markdown(f"**Percentage Error:** {pct_error:.2f}%")
            
            with metrics_col2:
                st.latex(r"\Delta x = \sqrt{SE^2 + LC^2}")
                st.latex(r"E_{rel} = \frac{\Delta x}{\bar{x}}")
            
            with st.expander("Show Raw LaTeX for Propagation"):
                st.code(r"""
\Delta x = \sqrt{SE^2 + LC^2} \\
E_{rel} = \frac{\Delta x}{\bar{x}} \\
E_{\%} = E_{rel} \times 100\%
                """, language="latex")
            
            st.markdown("##### Final Reported Result:")
            val_str = f"{mean_val:.4g}"
            unc_str = f"{total_abs_error:.4g}"
...
# ==========================================
    ...
    with display_col:
        ...
        with metrics_col:
            ...
            if len(x) > 2:
                # --- NEW: Robust Multi-Model Fitting Logic ---
                ...
                if fit_successful:
                    # Generic R^2 Calculation for all models
